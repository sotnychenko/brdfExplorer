...
projectToPCA::projectToPCA()
{}

projectToPCA::~projectToPCA()
{}

 float*  projectToPCA::reshape( )
 {
    ...
    // cout<< brdf[0]<<endl<<brdf[1]<<endl<<brdf[2]<<endl<<brdf[3]<<endl<<brdf[4]<<endl<<brdf[5];
     ...
     float* brdfFinal = new float[ numBRDFSamples * 3 ];
     int ind=0;
...
bool  projectToPCA::read_brdf( const char* filename )
{
...
float*    projectToPCA::reshape(double* CosineMap, bool* MaskMap,int mask_size,int Qsize, float* my_npy)
{
...
void    projectToPCA::MapBRDF(float* reshapedBRDF,float* median,int Qsize )
{
...
void   projectToPCA::UnmapBRDF(double* CosineMap,float* mappedData,bool* MaskMap,float* median,int mask_size)
...
void  projectToPCA::ProjectToPCSpace(float* data,float* PCs,float* relativeOffset,int Qsize)
...
bool projectToPCA::loadMERLData( const char* filename )
{
...
float* projectToPCA::project(const char *filename)
{
