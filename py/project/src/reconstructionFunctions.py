...
def ProjectToPCSpace(data,PCs,relativeOffset, eta=0):
    nPCs = 5    #Number of PCs to use (if only 5 values are known, only the first 5 PCs are used)
    ...
    #np.save('Q',A)
    U, s, Vt = np.linalg.svd(A, full_matrices=False) #Use scipy instead of numpy for SVD
    Ut = np.transpose(U)
    V = np.transpose(Vt)
